...
# Load the specific DecisionTree model
def load_decision_tree_model():
    """Load the DecisionTree model specifically"""
    model_filename = "SOC_DecisionTreeRegressor_v1.0_test_result_trial_end_v1.0_20250804_004228.joblib"
    model_path = os.path.join('models', model_filename)
    
    if os.path.exists(model_path):
        try:
            loaded_model = joblib.load(model_path)
            st.sidebar.success(f"✅ Model loaded: DecisionTreeRegressor")
            return loaded_model
        except Exception as e:
...
# Load the DecisionTree model for predictions
